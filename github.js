...
var filename = "./github-config.json";
    ...
    if(jsonObj !== null && typeof jsonObj === 'object') {
        ...
        var getProjectData = async function(actualObj) {
            ...
            if(issueArray.length != 0) {
                ...
                callback(projectkey, ownername, reponame, githubtoken, qtestusername, qtestdomain, qtestaccesstoken, dburl, accountname, issueArray, issuekeyArray, jiraProjectID, jiraProjectName, actualObj);
            }
            else {
...
getValues(function(projectkey, ownername, reponame, githubtoken, qtestusername, qtestdomain, qtestaccesstoken, dburl, accountname, issueArray, issuekeyArray, jiraProjectID, jiraProjectName, actualObj) {
    ...
    var deletefeatureFile = async function() {
        try {   
            ...
            if(result.meta.status == "200 OK") {
                ...
                logger.debug("Deleted existing feature files inside features/"+projectkey+" folder from github");
            }
            else {
    ...
    var getProjectID = async function() {
        ...
        if(result.meta.status == "201 Created") {
            logger.debug("Created feature files inside features/"+projectkey+" folder on "+reponame+" repository for github user "+ownername);
        }
        else {
            logger.error(new Error("Couldn't create feature files inside features/"+projectkey+" folder on "+reponame+" repository for github user "+ownername));
        }
        ...
        if(vcsresponse.meta.status == "200 OK") {
            ...
            logger.error(new Error("Couldn't retrieve vcsUrl, vcsBranch and vcsPrivateInformation from features/"+projectkey+" folder on "+reponame+" repository for github user "+ownername));
        }
        ...
        getAccountID(function(accountID) {
            ...
            objProject.vcs = {url: vcsUrl, type: 'github', branch: vcsBranch, featuresFolder: 'features/'+projectkey};
